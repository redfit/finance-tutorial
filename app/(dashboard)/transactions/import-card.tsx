...
const ImportCard = ({ data, onCancel, onSubmit }: Props) => {
  ...
  const handleContinue = () => {
    ...
    const mappedData = {
      ...
      body: body
        .map((row) => {
          ...
          return transformedRow.every((item) => index === null)
            ? []
            : transformedRow
        })
    ...
    const formattedData = arrayOfData
      .map((item) => {
        if (item.date && item.payee && item.date) {
          return {
            ...item,
      ...
      <Card className="border-none drop-shadow-sm">
        <CardHeader className="gap-y-2 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col lg:flex-row gap-y-2 items-center gap-x-2">
            <Button onClick={onCancel} size="sm" className="w-full lg:mx-auto">
              Cancel
            </Button>
            <Button
              size="sm"
