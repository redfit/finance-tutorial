...
export function onvertAmountFromMiliunits(amount: number) {
  return amount / 1000
}
...
export function fromatDateRange(period: Period) {
  const dafaultTo = new Date()
  const defaultFrom = subDays(dafaultTo, 30)
  if (!period?.from) {
    return `${format(defaultFrom, "LLL dd")} - ${format(dafaultTo, "LLL dd, y")}`
  }
